...
void UWB_callback(const uwb::UWB_msg& dis){
    ...
    if(dis.D0 > 0.2 && dis.D1 > 0.2 && dis.D2 > 0.2){
        ...
        elimanator.Input_UWB(dis.stamp.toSec(), Dis);
        elimanator.Input_VIO(estimator.latest_time, estimator.latest_P, estimator.latest_Q);
        estimator.correctTime = estimator.latest_time; //取VIO时间，processUWB()中已经通过时间辍对齐将时间统一到了VIO时间下
...
{
    outfile.open("/home/skbt/VIU_ROSBAG/txt_Data/Time.txt", std::ios::out);
