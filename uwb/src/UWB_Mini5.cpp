...
int main(int argc, char *argv[])
{
  ...
  ros::Publisher send_dis = Node.advertise<uwb::UWB_msg>("/UWB_module/distance", 10);
  //ros::Publisher send_err = Node.advertise<std_msgs::Float32>("/UWB_module/distance_err_ori", 10);
        ...
        if (bytes_read > 0) {
            ...
            if (buff[0] == 'm')
            {
                ...
                //if(dis_T0_A0 != 0 && dis_T0_A1 != 0 && dis_T0_A2 != 0){
                  ...
                  dis_msgs.stamp = ros::Time::now();
                  dis_msgs.D0 = dis_T0_A0;
                  err.data = dis_T0_A0 - 1.91;
                  dis_msgs.D1 = dis_T0_A1;
                  dis_msgs.D2 = dis_T0_A2;
                  send_dis.publish(dis_msgs);
                  //send_err.publish(err);
                //loop_rate.sleep();
                  //ROS_INFO("Publish distance[T0-A0: %f m, T0-A1: %f m, T0-A2: %f m]", dis_msgs.D0, dis_msgs.D1, dis_msgs.D2);
                }
            }
        }
    }
