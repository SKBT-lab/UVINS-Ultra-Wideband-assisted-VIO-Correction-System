...
int main(int argc, char *argv[])
{
  int serial_port = open("/dev/ttyUSB0", O_RDWR);
    //ros节点初始化
  ros::init(argc, argv, "uwb_module_SW2");

    //创建节点句柄
  ros::NodeHandle Node;
    
    //创建一个publisher，发布一个名为/turtle1/cmd_vel的topic, 消息类型为geometry_msgs::Twist， 队列长度为10
  ros::Publisher send_dis = Node.advertise<geometry_msgs::Vector3>("/UWB_module/distance_SW2", 10);
  ros::Publisher send_err_dis = Node.advertise<std_msgs::Float32>("/UWB_module/distanceA0_err_SW2", 10);
