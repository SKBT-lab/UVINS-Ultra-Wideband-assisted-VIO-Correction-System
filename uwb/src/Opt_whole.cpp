...
struct CostFunctor {
    CostFunctor( Eigen::Vector3d& x1,  double d)
        : x1_(x1), d_(d) {}

    template<typename T>
    bool operator()( T*  x,  T*  q, T* residuals) const {
        // 定义常量和变量
         Eigen::Matrix<T, 3, 1> x1_t = x1_.cast<T>();
         T d_t = static_cast<T>(d_);

        Eigen::Map<const Eigen::Matrix<T, 3, 1>> x_map(x);
        Eigen::Map<const Eigen::Quaternion<T>> q_map(q);

        // 计算旋转后的向量
        Eigen::Matrix<T, 3, 1> x2_rotated = q_map * UWB_IMU + x_map - A0;

        // 计算残差
        residuals[0] = (x1_t - x_map).norm() + (x2_rotated.norm() - d_t) * (x2_rotated.norm() - d_t);
...
void GetUWB_dis(const geometry_msgs::Vector3& uwb_dis){
   ...
   // 添加残差项
    ceres::CostFunction* cost_function =
        new ceres::AutoDiffCostFunction<CostFunctor, 1, 3, 4>(new CostFunctor(P_VIO, T0_A0));
